  ...
  return (req, res) => {
    ...
    const validateBody = body => {
      ...
      body.components.forEach(component => {
        if (
          [result, phenomenonTime, featureOfInterestId].indexOf(component) ===
          -1
        ) {
          throw Object.create({
            name: ERR.BAD_REQUEST,
      ...
      if (body[dataArrayIotCount] !== body.dataArray.length) {
        throw Object.create({
          name: ERR.BAD_REQUEST,
          errno: ERR.ERRNO_BAD_REQUEST,
          errors: '\'dataArray@iot.count\' doesn\'t match \'dataArray\' length.',
        })
      }
    ...
    const createInstances = observationObjects => {
      return db().then(models => {
        return Promise.all(
          observationObjects.map(observationObj => {
            return models
              .createInstance(observations, { body: observationObj })
              .then(instance => {
    ...
    const createObservations = body => {
      ...
      const featureOfInterestIndex = body.components.indexOf(
        featureOfInterestId
      )
    ...
    const createResponse = instances => {
      ...
      instances.forEach(instance => {
        if (instance) {
          msg = msg.concat(prepath + observations + '(' + instance + ')')
        } else {
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      observationObjects = observationObjects.concat(
        createObservations(body[i])
      )
    }

    createInstances(observationObjects)
      .then(instances => {
