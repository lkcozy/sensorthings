...
module.exports = (endpoint, port, mandatory, optional = []) => {
  ...
  return db().then(models => {
    ...
    const getCountObject = (url, body, empty, overrides) => {
      ...
      if (
        countObject[historicalLocations] &&
        !body[historicalLocations] &&
    ...
    describe('/' + endpoint + ' API', () => {
      describe('Preconditions', () => {
        beforeEach(done => {
          Promise.all(
            [models[endpoint].destroy({ where: {} })].concat(
              associatedModels.map(name => {
      ...
      describe('GET /' + endpoint + '(:id)', () => {
        ...
        it(
          'should respond 200 with a ' + endpoint + ' list if no id provided',
          done => {
            server
              .get(prepath + endpoint)
              .expect('Content-Type', /json/)
              ...
              .end((err, res) => {
                ...
                res.body.value.forEach(value => {
                  ...
                  const selfLink =
                    fullPrepath + endpoint + '(' + instanceId + ')'
                  const selfLink2 =
        ...
        it(
          'should respond 200 with a ' + endpoint + ' if id provided',
          done => {
            const property = mandatory[0]
            let anotherInstance = Object.assign({}, testEntity)
            anotherInstance[property] = anotherValue(property)
              ...
              new Promise(resolve => {
                server
                  ...
                  .end((err, res) => {
                    ...
                    value[CONST.iotSelfLink]
                      .indexOf(fullPrepath + modelName)
        ...
        ;[false, true].forEach(withValue => {
          describe(
            'Get specific properties' + (withValue ? ' with value' : ''),
            () => {
              it('should respond 404 getting an invalid property', done => {
                let path =
                  prepath + endpoint + '(' + instanceId + ')/randomProperty'
                if (withValue) {
                  path += '/$value'
                }
                server
              ...
              mandatory.concat(optional).forEach(property => {
                it(
                  'should return only 1 field when getting ' + property,
                  done => {
                    let path =
                      prepath + endpoint + '(' + instanceId + ')/' + property
                    if (withValue) {
                      path += '/$value'
                    }
                    server
              ...
              optional.forEach(property => {
                it(
                  'should return 204 when getting a empty ' + property,
                  done => {
                    let path =
                      prepath + endpoint + '(' + instanceId + ')/' + property
                    let updateProperty = {}
      ...
      describe('POST /' + endpoint, () => {
        ...
        postSuccess = (done, body, expected, resourceOverride) => {
          server
            .post(resourceOverride || resource)
            .send(body)
            .expect('Content-Type', /json/)
            .expect(201)
            .end((err, res) => {
        ...
        mandatory.forEach(property => {
          it(
            'should respond 400 if missing ' + property + ' property',
            done => {
        ...
        describe('Relations linking', () => {
          ...
          associatedModels.forEach(name => {
            it(
              'should respond 400 if request tries to create a ' +
                endpoint +
                ' linked to an unexisting ' +
                name,
              done => {
                let body = Object.assign({}, testEntity)
            ...
            it(
              'should respond 201 if request to link ' +
                endpoint +
                ' to existing ' +
                name +
                ' is valid',
                ...
                models[name].create(CONST[name + 'Entity']).then(relation => {
                  ...
                  const countObject = getCountObject(
                    resource,
                    body,
                    false,
                    overrides
            ...
            it(
              'should respond 201 if request to link ' +
                ...
                models[name].create(CONST[name + 'Entity']).then(instance => {
            ...
            it(
              'should respond 201 if request to create ' +
                endpoint +
                ' with related ' +
                name +
                ' is valid',
            ...
            it(
              'should respond 201 linking an array of ids in a hasMany/' +
            ...
            it(
              'should respond 201 linking an array of entities in a hasMany/' +
                'belongsToMany association',
              done => {
                const association = associations[associationsMap[name]]
                const type = association.associationType
            ...
            it(
              'should respond 400 if one of the ids in a linked array does ' +
                ' not exist',
              done => {
                const association = associations[associationsMap[name]]
                const type = association.associationType
            ...
            it(
              'should respond 400 linking an array in a hasOne/belongsTo' +
                ' association',
              done => {
                const association = associations[associationsMap[name]]
                const type = association.associationType
      ...
      describe('PATCH /' + endpoint + '(:id)', () => {
          ...
          server
            ...
            .end((err, res) => {
              ...
              res.header.location.should.be.equal(
                fullPrepath + endpoint + '(' + res.body[CONST.iotId] + ')'
              )
        ...
        it(
          'should respond 404 if request tries to update ' +
            'a ' +
            endpoint +
            ' that does not exist',
          done => {
        ...
        it(
          'should respond 200 if request to update a ' +
            'single property of a ' +
            endpoint +
            ' is valid',
          done => {
            const entity = Object.assign({}, testEntity)
        ...
        it(
          'should respond 200 if request to update all ' +
            'the properties of a ' +
            endpoint +
            ' is valid',
          done => {
        ...
        describe('Relations linking', () => {
          ...
          beforeEach(done => {
            ...
            Promise.all(promises).then(results => {
              results.forEach(result => {
                testEntities[models.getPlural(result.$modelOptions)] = result.id
              })
              done()
            })
          })
          ...
          associatedModels.forEach(name => {
            it(
              'should respond 400 if request tries to update a ' +
                endpoint +
                ' to link it to an unexisting ' +
                name,
              done => {
                let body = Object.assign({}, testEntity)
                removeAssociations(body)
            ...
            it(
              'should respond 400 if request includes related ' +
                ...
                patchError(
                  ...
                  body,
                  400,
                  ERR.ERRNO_INLINE_CONTENT_NOT_ALLOWED,
                  ERR.BAD_REQUEST
                )
              }
            )
            ...
            it(
              'should respond 200 if request to update a ' +
                endpoint +
                ' to link it to several existing ' +
                name +
                ' is correct',
              done => {
                const association = associations[associationsMap[name]]
            ...
            it(
              'should respond 400 to a request to update a ' +
                endpoint +
                ' to link it to several existing ' +
                name,
              done => {
      ...
      describe('DELETE /' + endpoint + '(:id)', () => {
        ...
        const deleteSuccess = done => {
          server
            ...
            .end((err, res) => {
              should.not.exist(err)
              res.status.should.be.equal(200)
              Promise.all([models[endpoint].findAndCountAll()]).then(
                results => {
        ...
        beforeEach(done => {
          ...
          let model
          model = models[endpoint]
          Promise.all([model.destroy({ where: {} })]).then(() => {
            const entity = Object.assign({}, testEntity)
            model.create(entity).then(instance => {
              instanceId = instance.id
              done()
        ...
        it(
          'should respond 404 if request tries to delete a ' +
            endpoint +
            ' that does not exist',
          done => {
            instanceId = 0
        ...
        it(
          'should respond 200 if request to delete a ' +
            endpoint +
            ' is ' +
            ' valid',
          done => {
        ...
        describe('Integrity constraints', () => {
          ...
          it(
            'should delete all ' +
              linkedModels +
              ' entities linked to the ' +
              endpoint +
              ' entity being deleted',
            done => {
              Promise.all(
                linkedModels.map(linkedModel => {
                ...
                return new Promise(resolve => {
                  ...
                  server
                    .delete(prepath + endpoint + '(' + id + ')')
                    .send()
                    .expect(200)
                    .end(err => {
                      should.not.exist(err)
                      Promise.all(
                        linkedModels.concat(endpoint).map(modelName => {
