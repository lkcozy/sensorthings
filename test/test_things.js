...
commonTests(CONST.things, 8888, mandatory, optional).then(tester => {
  describe('Things API - specific', () => {
    ...
    describe('Locations with same encodingType', () => {
      ...
      it(
        'should not allow to create 2 Locations related to same thing with ' +
          'the same encodingType',
        done => {
          let body = Object.assign({}, CONST.LocationsEntity)
          tester.server
            .post('/v1.0/Things(' + id + ')/Locations')
            .send(body)
      ...
      it(
        'should replace the association when patching a Thing with a new ' +
          'Location with the same encodingType',
        done => {
          db().then(models => {
            const loc = Object.assign({}, CONST.LocationsEntity)
            models.Locations.create(loc).then(locInstance => {
              let body = {
                Locations: [{ '@iot.id': locInstance.id }],
              }
              tester.server
                .patch('/v1.0/Things(' + id + ')')
                .send(body)
                .expect('Content-Type', /json/)
                .expect(200)
                .end((err, res) => {
    ...
    describe('HistoricalLocations', () => {
      it(
        'should create a new HistoricalLocations when patching the Thing ' +
          'with a new Location',
        done => {
          ...
          tester.server
            ...
            .end((err, res) => {
              should.not.exist(err)
              db().then(models => {
                models.Locations.create(CONST.LocationsEntity).then(
                  instance => {
                    const newBody = {
                    ...
                    tester.server
                      ...
                      .expect(200)
                      .end(() => {
                        models.HistoricalLocations.findAndCountAll().then(
                          result => {
    ...
    describe('Get FeaturesOfInterest from Locations', () => {
      ...
      it(
        'should respond 201 when inserting a Thing with a Observation ' +
          'FeatureOfInterest, but with Locations',
        done => {
